...
class Person:
    ...
    def from_random(cls, firstnames: list[str], surnames: list[str]):
        return cls(first_name=choice(firstnames),
                   ...
                   age=randint(1, 100),
                   sex=choice([x for x in Sex]),
                   dna=Dna.from_random())

    @classmethod
    def from_string(cls, person_string: str, dna: str):
        elements = person_string.replace("(", ":").replace(",", ":").replace(
            ")", "").split(":")
        return cls(first_name=elements[0],
    ...
    def __str__(self) -> str:
        return f"{self.first_name}:{self.last_name}({self.age},{self.sex.value})"
