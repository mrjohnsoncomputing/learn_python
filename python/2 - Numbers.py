...
# result = 1 + 1
# In the space below, create an appropriately named variable and set its value to the calculation 300 divided by 19
# In programming the forward slash character "/" is used to represent divide, and the asterix character "*" is used to represent multiply.
...
# The way a computer stores and performs calclations on an integer is different to that of a float, and so it is important we understand the difference.
# What number type is the result from 2.2?
# Type your answer as a string in the print() command in the space below.

# v 2.3 - Code v #

### ^^^^^^^^^^ ###

# We can convert one type of number to another. Putting a float inside the brackets of int() will convert our float into an integer.
# Try updating the code you wrote in 2.2 to have the answer print out as an integer rather than a decimal.
...
# Lets break down the code for the solution for the last section.
# Your answer should have looked like this print(int(result)) [Your variable name might not be result, that's fine!]
...
# Before running the code, predict what the answer will be. 
