...
# For example input("Please enter your username")
# In the space below, use the input command and ask the user for the year they were born
...
# Do this in the space below, and remember - variable names on the left of the equals and values on the right.
...
# This is important because the following - "2" * "2" || "2" * 2 || 2 * 2 - all give different results.
...
# Combine int() with your code from 3.2 and put in in the space below.
...
# Create a variable in the space below, using an appropriate variable name, and assign the value of the current year to it.
...
# print("You are " + age + "years old") [age is what i would have called the variable in 3.5, you might have called it something different and that is fine]
...
# If we try to do this, python will give us an error. However, just like we used the int() command to change a string to an integer...
# ...we can use the str() command to convert an integer into a string.
# Add the final line of code, printing how old they are in years. [The number by itself is not enough, you must be using additional strings like in my example above]
